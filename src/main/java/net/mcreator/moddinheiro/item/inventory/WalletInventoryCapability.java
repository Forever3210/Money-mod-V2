...
package net.mcreator.moddinheiro.item.inventory;
...
public class WalletInventoryCapability implements ICapabilitySerializable<CompoundTag> {
	...
	public static void onItemDropped(ItemTossEvent event) {
		if (event.getEntityItem().getItem().getItem() == ModDinheiroModItems.WALLET.get()) {
	...
	private ItemStackHandler createItemHandler() {
		return new ItemStackHandler(8) {
			...
			public boolean isItemValid(int slot, @Nonnull ItemStack stack) {
				return stack.getItem() != ModDinheiroModItems.WALLET.get();
			}
